...
Template( "cb_table.csv", row =>
    newTrial( "trial",
        newTimer(500)
            .start()
            .wait()
        ,
        newText( `${row.Matrix} ${row.Token} are ${row[row.Predicate.replace("Category",row.WhichCategory+'Category')]}` )
            .center()
            .print()
        ,
        newText("<p> Press <strong>F</strong> to <strong>Agree</strong> or <strong>J</strong> to <strong>Disagree</strong><p>")
    ...
    .log( "Negation" , row.Negation )
    .log( "CompTruth" , row.CompTruth )
    .log( "SubStat" , row.SubStat )
    .log( "Token" , row.Token )
...
Template( "cb_table.csv", row =>
    ["test",
        "DashedSentence", {s: `${row.Matrix} ${row.Token} are ${row[row.Predicate.replace("Category",row.WhichCategory+'Category')]}`},
        "PennController", newTrial("question",    
            newText("<p> Press <strong>F</strong> to <strong>Agree</strong> or <strong>J</strong> to <strong>Disagree</strong><p>")
                .center()
                .print()
...
newTrial("end",
    newText("You have just completed the second part of the experiment!")
        ...
        .print()
    ,
    newText("<p>If you need to take a break, do so before proceeding to the third and final part.</p>")
        .center()
        .print()
    ,
